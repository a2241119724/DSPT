...
class Flickr8kDataset(Dataset):
    def __init__(self, feature_path:str, caption_path:str):
        ...
        self.flicker8k_test_ids = np.load('./annotations/flicker8k_test_ids.npy')
    ...
    def split(self, split):
        ...
        if split == 'train':
            self.flicker8k_ids = self.flicker8k_train_ids
        elif split == 'val':
            self.flicker8k_ids = self.flicker8k_val_ids
        elif split == 'test':
            self.flicker8k_ids = self.flicker8k_test_ids

    def __getitem__(self, index):
        id = str(self.flicker8k_ids[index])
        data = np.array(self.f_grid[id + '_grids'])
        return torch.tensor(data), self.captions[id]
    
    def build_vocab(self, min_freq):
        if os.path.isfile('flicker8k_vocab.pkl'):
            print('Loading from vocabulary')
            vocab = pickle.load(open('flicker8k_vocab.pkl', 'rb'))
        ...
        with open('flicker8k_vocab.pkl', 'wb') as f:
            pickle.dump(vocab, f)
        self.text_field.vocab = vocab
        return vocab
