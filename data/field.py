...
class ImageField(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, feature_path=None, max_detections=100,
                 sort_by_prob=False, load_in_tmp=True):
        self.max_detections = max_detections
        self.feature_path = feature_path
        self.sort_by_prob = sort_by_prob

        tmp_detections_path = os.path.join('/tmp', os.path.basename(feature_path))

        if load_in_tmp:
            if not os.path.isfile(tmp_detections_path):
                if shutil.disk_usage("/tmp")[-1] < os.path.getsize(feature_path):
                    warnings.warn('Loading from %s, because /tmp has no enough space.' % feature_path)
                else:
                    warnings.warn("Copying detection file to /tmp")
                    shutil.copyfile(feature_path, tmp_detections_path)
                    warnings.warn("Done.")
    ...
    def preprocess(self, x, avoid_precomp=False):
        ...
        try:
            regions = boxes = sizes = np.random.rand(0)
            if("coco_all_align" in self.f_grid.filename or "X101_grid_feats_coco_trainval" in self.f_grid.filename):
                grids = self.f_grid['%d_grids' % image_id][()]
            elif("X152_trainval" in self.f_grid.filename or "swin_feature" in self.f_grid.filename):
                grids = self.f_grid['%d_features' % image_id][()]
            if self.sort_by_prob:
                regions = regions[np.argsort(np.max(self.f_region['%d_cls_prob' % image_id][()], -1))[::-1]]
        except KeyError:
            warnings.warn('Could not find detections for %d' % image_id)

        delta = self.max_detections - regions.shape[0]
        if delta > 0:
            regions = np.concatenate([regions, np.zeros((delta, regions.shape[1]))], axis=0)
        elif delta < 0:
            regions = regions[:self.max_detections]

        delta = self.max_detections - boxes.shape[0]
        if delta > 0:
