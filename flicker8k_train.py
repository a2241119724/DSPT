...
from data.flicker8kDataset import Flickr8kDataset, Flickr8kDataLoader
...
if __name__ == '__main__':
    ...
    for e in range(start_epoch, start_epoch + 100):
        ...
        if not args.only_test:
            ...
            dataset.split("train")
            if not use_rl:
                dataloader = Flickr8kDataLoader(dataset, batch_size=args.batch_size//5, shuffle=True, num_workers=args.workers)
                ...
                with tqdm(desc='Epoch %d - training' % e, unit='it', total=len(dataloader)) as pbar:
                    for it, (images, captions) in enumerate(iter(dataloader)):
                        ...
                        + torch.abs(torch.cosine_similarity(enc_output.unsqueeze(1),enc_output.unsqueeze(2),-1)).sum() * 1.1e-7
                ...
                dataloader = Flickr8kDataLoader(dataset, batch_size=args.rl_batch_size//5, shuffle=True, num_workers=args.workers)
        ...
        model.eval()
        dataset.split("val")
        dataloader = Flickr8kDataLoader(dataset, batch_size=10, shuffle=False, num_workers=args.workers)
        with tqdm(desc='Epoch %d - validating' % e, unit='it', total=len(dataloader)) as pbar:
            for it, (images, captions) in enumerate(iter(dataloader)):
                images = images.to(device)
                with torch.no_grad():
