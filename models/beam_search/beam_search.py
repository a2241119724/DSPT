...
class BeamSearch(object):
    ...
    def apply(self, regions, grids, boxes, sizes, out_size=1, return_probs=False, **kwargs):
        self.b_s = utils.get_batch_size(grids)
        self.device = utils.get_device(grids)
        ...
        with self.model.statefulness(self.b_s):
            for t in range(self.max_len):
                outputs,enc_output = self.iter(t, regions, grids, boxes, sizes, outputs, return_probs, **kwargs)
    ...
    def iter(self, t: int, regions, grids, boxes, sizes, outputs, return_probs, **kwargs):
        cur_beam_size = 1 if t == 0 else self.beam_size

        word_logprob,enc_output = self.model.step(t, self.selected_words, regions, grids, boxes, sizes, mode='feedback', **kwargs)
        ...
        selected_beam = torch.div(selected_idx, candidate_logprob.shape[-1], rounding_mode="floor") # no
        selected_words = selected_idx - selected_beam * candidate_logprob.shape[-1] # no

        self.model.apply_to_states(self._expand_state(selected_beam, cur_beam_size))
        # grids = self._expand_visual(grids, cur_beam_size, selected_beam)
